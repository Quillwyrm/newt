...
local function expect_throw(fn, needle)
    local ok_run, err = pcall(fn)
    if ok_run then
        fail("expected throw")
    end
...
test("graphics.set_default_filter", "bad token throws before init", function()
    ...
    end, "graphics.set_default_filter")
end)
...
local function run_globals_tests()
    ...
    test("rgba", "string forms parse hex", function()
    ...
    test("free", "non-userdata is a no-op", function()
        expect_nothrow(function()
            free(123)
            free("x")
            free(nil)
    ...
    test("free/graphics.get_image_size", "freed image query becomes nil,nil", function()
    ...
    test("free/graphics.get_pixelmap_size", "freed pixelmap query becomes nil,nil", function()
    ...
    test("free/audio.get_sound_info", "freed sound query becomes nil,nil,nil", function()
...
local function run_filesystem_tests()
    ...
    test("filesystem.*", "representative strict arity checks throw", function()
        expect_throw(function() filesystem.get_resource_dir(1) end, "filesystem.get_resource_dir")
        expect_throw(function() filesystem.get_working_dir(1) end, "filesystem.get_working_dir")
        expect_throw(function() filesystem.set_working_dir() end, "filesystem.set_working_dir")
        expect_throw(function() filesystem.get_args(1) end, "filesystem.get_args")
        expect_throw(function() filesystem.list_dir() end, "filesystem.list_dir")
        expect_throw(function() filesystem.info() end, "filesystem.info")
        expect_throw(function() filesystem.read_file() end, "filesystem.read_file")
        expect_throw(function() filesystem.write_file("x") end, "filesystem.write_file")
        expect_throw(function() filesystem.mkdir() end, "filesystem.mkdir")
        expect_throw(function() filesystem.rename("x") end, "filesystem.rename")
        expect_throw(function() filesystem.remove() end, "filesystem.remove")
    end)
    ...
    test("filesystem.read_file", "missing path returns nil, err", function()
        ...
        expect_nil_err(data, err, "filesystem.read_file missing")
    end)
    ...
    test("filesystem.write_file", "bad path returns false, err", function()
        ...
        expect_false_err(ok_write, err, "filesystem.write_file bad path")
    end)
    ...
    test("filesystem.list_dir", "missing dir returns nil, err", function()
        ...
        expect_nil_err(items, err, "filesystem.list_dir missing")
    end)
    ...
    test("filesystem.info", "missing path returns nil, err", function()
        ...
        expect_nil_err(info, err, "filesystem.info missing")
    end)
    ...
    test("filesystem.rename", "missing path returns false, err", function()
        ...
        expect_false_err(ok_rename, err, "filesystem.rename missing")
    end)
    ...
    test("filesystem.remove", "missing path returns false, err", function()
        ...
        expect_false_err(ok_remove, err, "filesystem.remove missing")
    end)
end

local function run_graphics_asset_tests()
    section("graphics / assets and cpu-side")
    ...
    test("graphics.new_canvas", "rejects non-positive sizes", function()
        expect_throw(function() graphics.new_canvas(0, 64) end, "graphics.new_canvas")
        expect_throw(function() graphics.new_canvas(-1, 64) end, "graphics.new_canvas")
    end)
    ...
    test("graphics.set_canvas", "accepts canvas nil and no-arg, rejects normal image", function()
        ...
        expect_throw(function() graphics.set_canvas(state.assets.image) end, "graphics.set_canvas: image is not a render target")
        free(canvas)
    end)

    test("free/graphics.get_image_size", "freed canvas query becomes falsey", function()
    ...
    test("graphics.pixelmap_set_pixel/get_pixel", "set/get and OOB get work", function()
        local pmap = new_blank_pixelmap(4, 4)
    ...
    test("free/graphics.pixelmap_get_pixel/get_cptr", "freed pixelmap queries become falsey", function()
    ...
    test("graphics.save_pixelmap/load_pixelmap", "save/load roundtrip works", function()
    ...
    test("graphics.pixelmap_flood_fill", "changes bounded region and no-ops on OOB/same-color", function()
    ...
    test("graphics.blit_*", "geometry op smoke", function()
...
local function run_audio_tests_init()
    section("audio / init-side")

    test("audio.config_bus_delay_times", "throws after init", function()
        expect_throw(function()
            audio.config_bus_delay_times({ [1] = 0.25 })
        end, "audio.config_bus_delay_times: must be called before engine initialization")
    end)
    ...
    test("audio.play/audio.play_at", "return handles", function()
        local h1, err1 = audio.play(state.assets.sfx, 1, 0.5, 1.0, 0.0)
        ok(h1, err1)
        is_type(h1, "number")
        audio.stop_voice(h1)

        local h2, err2 = audio.play_at(state.assets.bgm, 2, 100, 200, 0.5, 1.0)
        ok(h2, err2)
        is_type(h2, "number")
        audio.stop_voice(h2)
    end)
    ...
    test("audio.play/audio.play_at", "invalid bus throws", function()
        expect_throw(function() audio.play(state.assets.sfx, 99) end, "audio.play: invalid bus index")
        expect_throw(function() audio.play_at(state.assets.bgm, 99, 0, 0) end, "audio.play_at: invalid bus index")
    end)

    test("audio.set_listener_*/set_default_*", "listener and default setters work", function()
        ...
        expect_throw(function() audio.set_default_falloff_mode("bad") end, "audio.set_default_falloff_mode")
    end)

    test("free/audio.get_sound_info/audio.play/audio.play_at", "freed sound becomes falsey and cannot play", function()
...
local function run_window_tests()
    ...
    test("window.set_title/set_size/set_position", "setters work", function()
    ...
    test("window.set_flags", "valid forms work and bad token throws", function()
        ...
        expect_throw(function() window.set_flags({ "badflag" }) end, "window.set_flags")
    ...
    test("window.set_cursor/cursor_show/cursor_hide/is_cursor_visible", "cursor API works", function()
        ...
        expect_throw(function() window.set_cursor("bad") end, "window.set_cursor")
    ...
    test("window.set_clipboard/get_clipboard", "roundtrip works and rejects NUL", function()
        ...
        expect_throw(function() window.set_clipboard("a\0b") end, "window.set_clipboard")
    end)
end
...
local function run_input_tests()
    ...
    test("input.*", "representative strict arity checks throw", function()
        expect_throw(function() input.get_mouse_position(1) end, "input.get_mouse_position")
        expect_throw(function() input.get_mouse_wheel(1) end, "input.get_mouse_wheel")
        expect_throw(function() input.start_text(1) end, "input.start_text")
        expect_throw(function() input.stop_text(1) end, "input.stop_text")
        expect_throw(function() input.get_text(1) end, "input.get_text")
    end)
    ...
    test("input.repeated/down/pressed/released", "mouse repeated rejects and bad tokens throw", function()
        expect_throw(function() input.repeated("mouse1") end, "input.repeated")
        expect_throw(function() input.down("__bad__") end, "input.down")
        expect_throw(function() input.pressed("__bad__") end, "input.pressed")
        expect_throw(function() input.repeated("__bad__") end, "input.repeated")
        expect_throw(function() input.released("__bad__") end, "input.released")
    end)
    ...
    test("input.start_text/stop_text/get_text", "text API works", function()
...
local function run_audio_tests_update_step0()
    section("audio / runtime")

    test("audio voice handle API", "voice lifecycle and stale handle behavior", function()
        ...
        expect_throw(function() audio.seek_voice(handle, 0, "bad") end, "audio.seek_voice")
        ...
        expect_throw(function() audio.set_voice_falloff_mode(handle, "bad") end, "audio.set_voice_falloff_mode")
        expect_throw(function() audio.set_voice_pan_mode(handle, "bad") end, "audio.set_voice_pan_mode")
    ...
    test("audio bus API", "bus controls, invalid bus rules, and stop semantics work", function()
        ...
        expect_throw(function() audio.set_bus_volume(99, 1.0) end, "audio.set_bus_volume")
        expect_throw(function() audio.set_bus_pitch(99, 1.0) end, "audio.set_bus_pitch")
        expect_throw(function() audio.set_bus_pan(99, 0.0) end, "audio.set_bus_pan")
        expect_throw(function() audio.fade_bus(99, 0.0, 0.1) end, "audio.fade_bus")
        expect_throw(function() audio.pause_bus(99) end, "audio.pause_bus")
        expect_throw(function() audio.resume_bus(99) end, "audio.resume_bus")
        expect_throw(function() audio.stop_bus(99) end, "audio.stop_bus")

        expect_throw(function() audio.set_bus_lpf(0, 1000) end, "audio.set_bus_lpf")
        expect_throw(function() audio.set_bus_hpf(0, 1000) end, "audio.set_bus_hpf")
        expect_throw(function() audio.set_bus_delay_mix(0, 0.5, 0.5) end, "audio.set_bus_delay_mix")
        expect_throw(function() audio.set_bus_delay_feedback(0, 0.5) end, "audio.set_bus_delay_feedback")
    ...
    test("audio.get_voice_info", "time progression setup", function()
...
local function run_audio_tests_update_step1()
...
local function run_draw_tests()
    section("graphics / draw")

    test("graphics.clear/draw_rect/debug_line/debug_rect/debug_text", "core draw calls do not throw", function()
        graphics.clear()
    ...
    test("graphics.set_blend_mode", "accepts valid tokens and rejects bad token", function()
        local modes = { "replace", "blend", "add", "multiply", "modulate", "premultiplied" }
        for i = 1, #modes do
            graphics.set_blend_mode(modes[i])
        end
        expect_throw(function() graphics.set_blend_mode("bad") end, "graphics.set_blend_mode")
        graphics.set_blend_mode("blend")
    end)
    ...
    test("graphics.draw_image/draw_image_region", "image draw calls work and nil contract is loud", function()
        ...
        expect_throw(function() graphics.draw_image(nil, 0, 0) end, "graphics.draw_image")
        expect_throw(function() graphics.draw_image_region(nil, 0, 0, 1, 1, 0, 0) end, "graphics.draw_image_region")
    end)

    test("graphics.draw_image/draw_image_region", "drawing a freed image is a no-op", function()
        local img, err = graphics.load_image(ASSET_IMG)
        ok(img, err)
        free(img)
    ...
    test("graphics.begin_transform/end_transform", "underflow and overflow throw", function()
        expect_throw(function() graphics.end_transform() end, "graphics.end_transform")
        for _ = 1, 31 do
            graphics.begin_transform()
        end
        expect_throw(function() graphics.begin_transform() end, "graphics.begin_transform")
        for _ = 1, 31 do
            graphics.end_transform()
        end
    end)
    ...
    test("graphics.set_canvas/draw_rect/draw_image", "can draw to canvas and return to screen", function()
        local canvas = new_canvas_or_fail(16, 16)
...
local function finalize_and_exit()
    ...
    print(("skipped: %d"):format(Runner.skipped))
