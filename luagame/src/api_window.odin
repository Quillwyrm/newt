...
lua_window_init :: proc "c" (L: ^lua.State) -> c.int {
	...
	if Window != nil {
		lua.L_error(L, cstring("window.init: already initialized"))
		return 0
	}

	nargs := lua.gettop(L)
	if nargs < 3 {
		lua.L_error(L, cstring("window.init expects: width, height, title, [flags]"))
		return 0
	}
	...
	if Renderer == nil {
		fmt.eprintln("Renderer failed:", sdl.GetError())
		return 0
	}
...
register_window_api :: proc(L: ^lua.State) {
	...
	lua.setfield(L, -2, cstring("get_size"))

	lua.pushcfunction(L, lua_window_get_position)
	lua.setfield(L, -2, cstring("get_position"))
	...
	lua.setglobal(Lua, cstring("window"))
