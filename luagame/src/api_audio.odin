...
claim_and_init_voice :: proc(sound: ^Sound, bus_idx: int) -> (^Voice, u32, cstring, bool) {
	...
	if voice_idx == -1 {
		if oldest_idx != -1 {
			...
			return nil, 0, cstring("voice pool exhausted (all active voices are streaming)"), false
		}
	}
	...
	if sound.is_stream {
		result = ma.sound_init_from_file(
			...
			sound.filepath,
			flags,
			&audio_ctx.mixer[bus_idx].group,
			nil,
			&voice.node,
		)
	} else {
		result = ma.sound_init_copy(
			&audio_ctx.engine,
			&sound.cache_ref,
			flags,
			&audio_ctx.mixer[bus_idx].group,
...
lua_audio_set_default_falloff_mode :: proc "c" (L: ^lua.State) -> c.int {
	...
	case:
		lua.L_error(
			L,
			"audio.set_default_falloff_mode: expected 'none', 'inverse', 'linear', or 'exponential'",
		)
		return 0
	}
...
lua_audio_set_voice_falloff_mode :: proc "c" (L: ^lua.State) -> c.int {
	...
	case:
		lua.L_error(
			L,
			"audio.set_voice_falloff_mode: expected 'none', 'inverse', 'linear', or 'exponential'",
		)
		return 0
	}
...
lua_audio_fade_bus :: proc "c" (L: ^lua.State) -> c.int {
	...
	ma.sound_group_set_fade_in_milliseconds(
		&audio_ctx.mixer[idx].group,
		current_vol,
		target,
		duration_ms,
	)
...
lua_audio_set_bus_lpf :: proc "c" (L: ^lua.State) -> c.int {
	...
	if idx <= 0 || idx >= MAX_AUDIO_BUSES {
		lua.L_error(
			L,
			"audio.set_bus_lpf: bus index must be between 1 and %d",
			MAX_AUDIO_BUSES - 1,
		)
		return 0
	}
...
lua_audio_set_bus_hpf :: proc "c" (L: ^lua.State) -> c.int {
	...
	if idx <= 0 || idx >= MAX_AUDIO_BUSES {
		lua.L_error(
			L,
			"audio.set_bus_hpf: bus index must be between 1 and %d",
			MAX_AUDIO_BUSES - 1,
		)
		return 0
	}
...
lua_audio_set_bus_delay_feedback :: proc "c" (L: ^lua.State) -> c.int {
	...
	if idx <= 0 || idx >= MAX_AUDIO_BUSES {
		lua.L_error(
			L,
			"audio.set_bus_delay_feedback: bus index must be between 1 and %d",
			MAX_AUDIO_BUSES - 1,
		)
		return 0
	}
...
lua_audio_set_bus_delay_mix :: proc "c" (L: ^lua.State) -> c.int {
	...
	if idx <= 0 || idx >= MAX_AUDIO_BUSES {
		lua.L_error(
			L,
			"audio.set_bus_delay_mix: bus index must be between 1 and %d",
			MAX_AUDIO_BUSES - 1,
		)
		return 0
	}
